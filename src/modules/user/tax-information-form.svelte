<script lang="ts">
	...
	import { newTaxInformationSchema } from '$modules/user/schemas';
	import { defaults, superForm } from 'sveltekit-superforms';
	import { zod, zodClient } from 'sveltekit-superforms/adapters';
	import { createTaxInformation } from '$modules/user';
	import * as Form from '$lib/components/ui/form';
	import { LoaderIcon } from 'lucide-svelte';
	import { goto } from '$app/navigation';

	let enabledNewsletter = $state(false);
	let acknowledgedCertification = $state(false);
	...
	let { taxInformation }: Props = $props();

	const client = useQueryClient();

	const createTaxInformationMutation = createMutation({
		mutationKey: ['create-tax-information'],
		mutationFn: createTaxInformation,
		onSuccess: () => {
			client.invalidateQueries({ queryKey: ['tax-information'] });
			toast.success('Tax Information completed');
			goto('/settings');
		}
	});

	const form = superForm(defaults(zod(newTaxInformationSchema)), {
		SPA: true,
		validators: zodClient(newTaxInformationSchema),
		resetForm: true,
		onSubmit() {
			if (taxInformation) {
				$createTaxInformationMutation.mutate({
					...$formData
				});
			}
		}
	...
	function handleNext() {
		if ($formData.country && agreedToTerms) {
			currentStep = 2;
		}
	}
	...
	function handleDone() {
		if (!acknowledgedCertification) return;

		const payload = {
			...$formData,
			enabled_newsletter: enabledNewsletter,
			acknowledged_certification: acknowledgedCertification
		};

		$createTaxInformationMutation.mutate(payload);
	}
</script>

<form method="POST" use:enhance id="tax-information-form">
			...
			<div class="flex-1">
					...
					<Form.Field {form} name="country">
						<Form.Control>
							...
							<Dropdown
								...
								bind:selectedValue={$formData.country}
								options={COUNTRIES}
							/>
						</Form.Control>
						<Form.FieldErrors />
				...
				<div class="mt-6 space-y-2 rounded-xl border p-4">
					...
					<div class="flex items-start gap-3">
						<Checkbox id="newsletter" bind:checked={enabledNewsletter} class="mt-0.5" />
						<Label
							for="newsletter"
				...
				<div class="mt-6 flex justify-end gap-4">
					...
					<Button disabled={!$formData.country || !agreedToTerms} onclick={handleNext}>Next</Button>
				</div>
			</div>
		</div>
	{/if}
	...
	{#if currentStep === 2}
		<div class="mt-6 space-y-2 rounded-xl bg-gray-100 p-4">
			...
			<p>
				Under penalties of perjury, I declare that I have examined the information on this form and
				to the best of my knowledge and belief it is true, correct, and complete. I further certify
			...
			<ul class="list-disc space-y-2 pl-6">
				<li>
					...
					individual that is the beneficial owner) of all the income to which this form relates or
				...
				<li>
					The person named on line 1 of this form is a resident of the treaty country listed on line
					9 of the form (if any) within the meaning of the income tax treaty between the United
			...
			<p>
				...
				receipt, or custody of the income of which I am the beneficial owner or any withholding
				agent that can disburse or make payments of the income of which I am the beneficial owner. I
				agree that I will submit a new form within 30 days if any certification made on this form
				becomes incorrect.
			</p>
			<p>
				...
				individual and, if applicable, obtain a reduced rate of withholding.
			</p>
		</div>

		<div class="mt-6 space-y-2 rounded-xl border p-4">
			<div class="flex items-start gap-3">
				<Checkbox id="terms" bind:checked={acknowledgedCertification} class="mt-0.5" />
				<Label
					for="terms"
					class="text-muted-foreground cursor-pointer text-sm leading-relaxed font-normal"
				>
		...
		<div class="mt-6 flex justify-end gap-4">
			...
			<Form.Button
				onclick={handleDone}
				disabled={!acknowledgedCertification ||
					$createTaxInformationMutation.isPending ||
					$createTaxInformationMutation.isPaused}
			>
				{#if $createTaxInformationMutation.isPending || $createTaxInformationMutation.isPaused}
